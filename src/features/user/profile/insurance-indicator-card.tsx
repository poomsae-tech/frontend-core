...
export function InsuranceIndicatorCard({ status }: InsuranceIndicatorProps) {
  return (
    <div className="mt-6 flex w-full items-center justify-between gap-4 rounded-full bg-secondary px-6 py-4 flex-row">
      <Popover>
        <PopoverTrigger>
          <Info className="h-5 w-5 cursor-pointer hover:opacity-75 transition-opacity text-muted-foreground" />
        </PopoverTrigger>
